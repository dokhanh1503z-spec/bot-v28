...
class UltimateBotV36:
    ...
    def E_variation_series(self, seq, gene_based=False):
        if len(seq) < 30:
            return None
        if gene_based:
            gene = self.encode_gene(self.get_streaks(seq))
            if len(gene) < 30:
                return None
            # tính E từ gene trực tiếp: mỗi gene X,S,M,L tương ứng 1,2,3,4+ (rút gọn)
            values = []
            mapping = {"X": 1, "S": 2, "M": 3}
            for i in range(30, len(gene)):
                part = gene[:i]
                streak_values = []
                for g in part:
                    if g in mapping:
                        streak_values.append(mapping[g])
                    elif g.startswith("L"):
                        streak_values.append(int(g[1:]))
                e, _ = self.calculate_E(streak_values)
                if e is not None:
                    values.append(e)
            return values
        else:
    ...
    def E_to_direction(self, E):
        ...
        for i in range(1, len(E)):
            dirs.append(1 if E[i] > E[i - 1] else -1)
        return dirs
    ...
    def forecast(self, seq):
        ...
        return {
            ...
            "gene": " ".join(gene[-40:]),
            "E_series": E_series,
            "gene_series": gene  # ✅ lưu gene series để vẽ chart trực tiếp
        }
...
if "data" in st.session_state:
    ...
    # ---------- DATA COUNTERS ----------
    st.write(f"✅ Data từ Google Sheets: {len(raw_input)}")
    st.write(f"✅ Data dùng vẽ chart (số gene): {len(bot.cl_seq)}")
    ...
    # ---------- INPUT CHO CHART 1 ----------
    chart1_len = st.number_input("Nhập số lượng E để vẽ chart CL khung 1", min_value=10, max_value=1000, value=50, step=10)
    chart2_len = st.number_input("Nhập số lượng E để vẽ chart CL khung 2", min_value=10, max_value=1000, value=100, step=10)

    # ---------- VẼ 2 CHART cùng lúc ----------
    for idx, chart_len in enumerate([chart1_len, chart2_len], start=1):
        sub_seq = bot.cl_seq[-chart_len:]
        E_series_new = bot.E_variation_series(sub_seq, gene_based=True)
        if E_series_new:
            ...
            fig.update_layout(
                title=f"Biểu đồ E CL khung {idx} (Zoom kéo thả)",
    ...
    chart1_len_tn = st.number_input("Nhập số lượng E để vẽ chart TN khung 1", min_value=10, max_value=1000, value=50, step=10)
    chart2_len_tn = st.number_input("Nhập số lượng E để vẽ chart TN khung 2", min_value=10, max_value=1000, value=100, step=10)

    for idx, chart_len in enumerate([chart1_len_tn, chart2_len_tn], start=1):
        sub_seq = bot.tn_seq[-chart_len:]
        E_series_new = bot.E_variation_series(sub_seq, gene_based=True)
        if E_series_new:
            df = pd.DataFrame({"E": E_series_new})
            df["E=2"] = 2
            ...
            fig.add_hrect(y0=1.8, y1=2.2, opacity=0.1)
            fig.update_layout(
                title=f"Biểu đồ E TN khung {idx} (Zoom kéo thả)",
                xaxis=dict(rangeslider=dict(visible=True)),
                hovermode="x unified"
            )
            st.plotly_chart(fig, use_container_width=True)
