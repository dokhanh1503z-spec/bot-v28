...
class UltimateBotV36:
    ...
    # ------------------ CẬP NHẬT PHẦN VẼ CHART E ------------------
    def E_variation_series(self, seq, gene_based=False, gene_input_len=None):
        if gene_based:
            gene = self.encode_gene(self.get_streaks(seq))
            if gene_input_len:
                gene = gene[-gene_input_len:]  # lấy n gene cuối theo input
            if len(gene) == 0:
                return None, 0
            mapping = {"X":1,"S":2,"M":3}
            streak_values = []
            ...
            total_count = sum(streak_values)  # bộ đếm tổng số
            values = []
            for i in range(1,len(streak_values)+1):
                part = streak_values[:i]
                e,_ = self.calculate_E(part)
                values.append(e if e is not None else 0)
            ...
            for i in range(20, len(streaks)):
                ...
                if e is not None:
                    values.append(e)
            return values, sum(streaks)

    # ------------------ PHẦN FORECAST GIỮ NGUYÊN ------------------
    ...
    def E_trend_analysis(self, seq):
        E_series, _ = self.E_variation_series(seq)
...
if "data" in st.session_state:
    ...
    chart_len_gene = st.number_input("Nhập số lượng gene để vẽ chart CL", min_value=1, max_value=200, value=50, step=1)
    E_series_new, total_count = bot.E_variation_series(bot.cl_seq, gene_based=True, gene_input_len=chart_len_gene)
    ...
    if E_series_new:
        ...
        fig.update_layout(
            title=f"Biểu đồ E CL (Tổng số streak = {total_count})",
